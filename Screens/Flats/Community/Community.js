...
import CommunityBingoScreen from './Community/CommunityBingo';
import HelpdeskScreen from './Community/Helpdesk';
import NoticeBoardScreen from './Community/NoticeBoard';
import DailyHelpScreen from './Community/DailyHelp';
import SearchVehicleScreen from './Community/SearchVehicle';
import CommunicationScreen from './Community/Communication';
import RentParkingScreen from './Community/RentParking';
import EmergencyContactsScreen from './Community/EmergencyContacts';
import ResidentsScreen from './Community/Residents';
import LocalDirectoryScreen from './Community/LocalDirectory';
import ElectionsAndSurveyScreen from './Community/ElectionsAndSurvey';
import AmenitiesScreen from './Community/Amenities';
...
const row3Actions = [
  { label: 'Local Directory', icon: 'book-outline' },
  { label: 'Elections & Survey', icon: 'checkmark-circle-outline' },
  { label: 'Amenities', icon: 'building-outline' },
];
...
export default function CommunityScreen({ navigation }) {
  const [expandedActions, setExpandedActions] = React.useState(false);
  const [selectedAction, setSelectedAction] = React.useState(null);

  // Action mapping for navigation
  const actionScreenMap = {
    'Community Bingo': () => setSelectedAction('bingo'),
    'Helpdesk': () => setSelectedAction('helpdesk'),
    'Notice Board': () => setSelectedAction('notice'),
    'Daily Help': () => setSelectedAction('dailyhelp'),
    'Search Vehicle': () => setSelectedAction('vehicle'),
    'Communi...': () => setSelectedAction('communication'),
    'Rent Parking': () => setSelectedAction('parking'),
    "Emergency No's": () => setSelectedAction('emergency'),
    'Residents': () => setSelectedAction('residents'),
    'Local Directory': () => setSelectedAction('localdirectory'),
    'Elections & Survey': () => setSelectedAction('elections'),
    'Amenities': () => setSelectedAction('amenities'),
  };

  // Function to create goBack handler for action screens
  const handleGoBack = () => setSelectedAction(null);

  // Render action screens
  if (selectedAction === 'bingo') {
    return <CommunityBingoScreen navigation={{ goBack: handleGoBack }} />;
  }
  if (selectedAction === 'helpdesk') {
    return <HelpdeskScreen navigation={{ goBack: handleGoBack }} />;
  }
  if (selectedAction === 'notice') {
    return <NoticeBoardScreen navigation={{ goBack: handleGoBack }} />;
  }
  if (selectedAction === 'dailyhelp') {
    return <DailyHelpScreen navigation={{ goBack: handleGoBack }} />;
  }
  if (selectedAction === 'vehicle') {
    return <SearchVehicleScreen navigation={{ goBack: handleGoBack }} />;
  }
  if (selectedAction === 'communication') {
    return <CommunicationScreen navigation={{ goBack: handleGoBack }} />;
  }
  if (selectedAction === 'parking') {
    return <RentParkingScreen navigation={{ goBack: handleGoBack }} />;
  }
  if (selectedAction === 'emergency') {
    return <EmergencyContactsScreen navigation={{ goBack: handleGoBack }} />;
  }
  if (selectedAction === 'residents') {
    return <ResidentsScreen navigation={{ goBack: handleGoBack }} />;
  }
  if (selectedAction === 'localdirectory') {
    return <LocalDirectoryScreen navigation={{ goBack: handleGoBack }} />;
  }
  if (selectedAction === 'elections') {
    return <ElectionsAndSurveyScreen navigation={{ goBack: handleGoBack }} />;
  }
  if (selectedAction === 'amenities') {
    return <AmenitiesScreen navigation={{ goBack: handleGoBack }} />;
  }
    ...
    <SafeAreaView style={styles.safeArea}>
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.content}>
        ...
        <View style={styles.actionGrid}>
          {row1Actions.map((item) => (
            <TouchableOpacity
              ...
              activeOpacity={0.85}
              onPress={() => actionScreenMap[item.label]?.()}
            >
        ...
        <View style={styles.actionGrid}>
          ...
          {expandedActions && (
            <TouchableOpacity
              key={row3Actions[0].label}
              style={styles.actionItem}
              activeOpacity={0.85}
              onPress={() => actionScreenMap[row3Actions[0].label]?.()}
            >
              <View style={styles.actionIconBox}>
                <Ionicons name={row3Actions[0].icon} size={30} color="#173A42" />
              </View>
              <Text style={styles.actionLabel}>{row3Actions[0].label}</Text>
            </TouchableOpacity>
          )}
          {!expandedActions && (
            <TouchableOpacity
              style={styles.actionItem}
              activeOpacity={0.85}
              onPress={() => setExpandedActions(true)}
            >
              <View style={styles.actionIconBox}>
                <Ionicons name="chevron-down-outline" size={30} color="#173A42" />
              </View>
        ...
        {expandedActions && (
          <View style={[styles.actionGrid, styles.actionGridLeft]}>
            {row3Actions.slice(1).map((item) => (
              <TouchableOpacity
                key={item.label}
                style={styles.actionItem}
                activeOpacity={0.85}
            ...
            <TouchableOpacity
              style={styles.actionItem}
              activeOpacity={0.85}
              onPress={() => setExpandedActions(false)}
            >
              <View style={styles.actionIconBox}>
                <Ionicons name="chevron-up-outline" size={30} color="#173A42" />
              </View>
              <Text style={styles.actionLabel}>View Less</Text>
            </TouchableOpacity>
          </View>
        )}
...
const styles = StyleSheet.create({
  ...
  actionGrid: { flexDirection: 'row', flexWrap: 'wrap', justifyContent: 'space-around', paddingHorizontal: 10, marginBottom: 16, gap: 8 },
  actionGridLeft: { justifyContent: 'flex-start' },
  actionItem: { width: '17%', marginBottom: 24, alignItems: 'center' },
